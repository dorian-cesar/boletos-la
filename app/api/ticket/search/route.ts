...
const BACKEND_URL =
  process.env.DB_URL || "https://boletos-la-analitica.dev-wit.com";
  ...
  try {
    ...
    if (!ticketNumber) {
      return NextResponse.json(
        { success: false, message: "Número de ticket requerido" },
        { status: 400 },
      );
    }
    ...
    if (!token) {
      return NextResponse.json(
        { success: false, message: "Error de autenticación con el backend" },
        { status: 500 },
      );
    }
    ...
    const ticketRes = await fetch(
      `${BACKEND_URL}/api/tickets/number/${ticketNumber}`,
      {
        headers: {
    ...
    if (!ticketRes.ok) {
      ...
      console.error(
        "[Ticket Search] Fetch failed with status:",
        ticketRes.status,
      );
      return NextResponse.json(
        { success: false, message: "Error consultando el ticket" },
        { status: 500 },
      );
    }
    ...
    const rawCompany =
      ticketData.company_name || ticketData.company || ticketData.carrier || "";
    let companyFormatted = rawCompany;
    if (rawCompany) {
      const upperComp = String(rawCompany).toUpperCase();
      if (upperComp.includes("LSA"))
        companyFormatted = "La Santaniana Argentina";
      else if (upperComp.includes("LSN"))
        companyFormatted = "La Santaniana Nacional";
    ...
    const formattedData = {
      ...
      documentType:
        ...
        ticketData.passenger?.docType ||
        ticketData.docType ||
        "Documento",
      documentNumber:
        ticketData.document_number ||
        ticketData.passenger?.docNumber ||
