...
export default function MisViajesPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.message || "No se encontró ningún boleto con el número ingresado.");
      }
  ...
  return (
    <div className="min-h-screen bg-[#0f1419] flex flex-col text-white">
      ...
      <main className="flex-1 relative flex flex-col items-center pt-32 pb-20 px-4">
        ...
        <div className="relative z-10 w-full max-w-4xl animate-fade-in">
          <div className="text-center mb-10">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 text-balance">
              Mis <span className="text-primary">Viajes</span>
            </h1>
            <p className="text-white/60 text-lg max-w-md mx-auto">
              Gestioná tus reservas y descargá tus boletos electrónicos de forma rápida y segura.
            </p>
          </div>
            ...
            <Card className="p-6 md:p-8 bg-white/5 backdrop-blur-xl border-white/10 shadow-2xl overflow-hidden relative max-w-2xl mx-auto">
              <form onSubmit={handleSearch} className="relative space-y-6">
                <div className="relative">
                  <label htmlFor="ticketNumber" className="block text-sm font-medium text-white/70 mb-2 ml-1">
                    Número de Boleto
                  </label>
                  <div className="relative">
            ...
            <div className="space-y-8 animate-slide-up">
                ...
                <div className="relative bg-[#1a1f26] rounded-[1.5rem] overflow-hidden border border-white/10 shadow-2xl flex flex-col md:flex-row min-h-[400px]">
                  ...
                  <div className="flex-1 p-8 md:p-10 flex flex-col">
                    <div className="flex justify-between items-start mb-8">
                      <div className="space-y-1">
                        <p className="text-primary text-xs font-bold tracking-[0.2em] uppercase">Boleto Electrónico</p>
                        <h2 className="text-2xl font-bold">{ticketData.company}</h2>
                      </div>
                      <div className="text-right">
                        <p className="text-white/40 text-[10px] uppercase tracking-widest mb-1">N° Ticket</p>
                        <p className="font-mono font-bold text-lg text-secondary">{ticketData.ticket_number}</p>
                      </div>
                    </div>

                    <div className="flex flex-col md:flex-row items-center gap-6 mb-10">
                      <div className="flex-1 text-center md:text-left">
                        <p className="text-white/40 text-xs uppercase tracking-widest mb-2">Origen</p>
                        <h3 className="text-3xl font-bold mb-1">{ticketData.origin_title}</h3>
                        <p className="text-white/60 text-sm flex items-center justify-center md:justify-start gap-1">
                          <MapPin className="h-3 w-3" /> {ticketData.origin_id}
                        </p>
                      </div>
                      ...
                      <div className="flex-1 text-center md:text-right">
                        <p className="text-white/40 text-xs uppercase tracking-widest mb-2">Destino</p>
                        <h3 className="text-3xl font-bold mb-1">{ticketData.destination_title}</h3>
                        <p className="text-white/60 text-sm flex items-center justify-center md:justify-end gap-1">
                          <MapPin className="h-3 w-3" /> {ticketData.destination_id}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-auto">
                      <div>
                        ...
                        <p className="font-bold">{format(parse(ticketData.departure_date, "yyyy-MM-dd", new Date()), "dd MMM, yyyy", { locale: es })}</p>
                      </div>
                      <div>
                        <p className="text-white/40 text-[10px] uppercase tracking-widest mb-2 flex items-center gap-1">
                          <Clock className="h-3 w-3" /> Salida
                        </p>
                        <p className="font-bold">{ticketData.departure_time.substring(0, 5)} hs</p>
                      </div>
                      <div>
                        <p className="text-white/40 text-[10px] uppercase tracking-widest mb-2 flex items-center gap-1">
                          <User className="h-3 w-3" /> Asiento
                        </p>
                        <p className="font-bold text-primary">{ticketData.seat_number}</p>
                      </div>
                      <div>
                        <p className="text-white/40 text-[10px] uppercase tracking-widest mb-2 flex items-center gap-1">
                          <ShieldCheck className="h-3 w-3" /> Estado
                        </p>
                        <p className="font-bold text-green-400 capitalize">{ticketData.payment_status}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="w-full md:w-[300px] bg-white/[0.02] p-8 md:p-10 flex flex-col items-center justify-center text-center">
                    <div className="bg-white p-3 rounded-2xl mb-6 shadow-xl shadow-black/50">
                      {qrCodeUrl ? (
                        <Image src={qrCodeUrl} alt="Ticket QR" width={140} height={140} className="rounded-lg" />
                      ) : (
                        <div className="w-[140px] h-[140px] bg-gray-200 animate-pulse rounded-lg" />
                      )}
                    </div>
                    
                    <p className="text-[10px] text-white/40 uppercase tracking-widest mb-6">Escaneá al abordar</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="bg-white/5 border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                  <h4 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <User className="h-4 w-4 text-primary" /> Información del Pasajero
                  </h4>
                  <div className="space-y-4">
                    <div className="flex justify-between border-b border-white/5 pb-2">
                      <span className="text-white/60">Nombre completo</span>
                      <span className="font-bold">{ticketData.first_name} {ticketData.last_name}</span>
                    </div>
                    <div className="flex justify-between border-b border-white/5 pb-2">
                      <span className="text-white/60">{ticketData.document_type_name}</span>
                      <span className="font-bold">{ticketData.document_number}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/60">Nacionalidad</span>
                      <span className="font-bold">{ticketData.nationality}</span>
                    </div>
                  </div>
                </Card>
                ...
                <Card className="bg-white/5 border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                  <h4 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Info className="h-4 w-4 text-secondary" /> Detalles del Pago
                  </h4>
                  <div className="space-y-4">
                    <div className="flex justify-between border-b border-white/5 pb-2">
                      <span className="text-white/60">Monto pagado</span>
                      <span className="font-bold text-secondary">Gs. {ticketData.seat_price.toLocaleString("es-PY")}</span>
                    </div>
                    <div className="flex justify-between border-b border-white/5 pb-2">
                      <span className="text-white/60">Método</span>
                      <span className="font-bold capitalize">{ticketData.payment_status}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/60">Fecha de compra</span>
                      <span className="font-bold">{format(new Date(ticketData.created_at), "dd/MM/yyyy HH:mm")}</span>
                    </div>
                  </div>
                </Card>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                <div className="flex items-center gap-3 text-white/40 text-sm">
                  ...
                  Podes presentar este boleto desde tu celular
                </div>
              </div>
            </div>
          )}
