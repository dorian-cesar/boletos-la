  ...
  return (
    ...
    >
      <span
        ...
        className={cn(
          cn(
            "text-lg lg:text-base font-semibold whitespace-nowrap inline-block transition-transform",
...
const getParaguayDate = () => {
  const asuncionTimeStr = new Date().toLocaleString("en-US", {
...
export function ParaguaySearchForm({
  ...
  const { availableDestinations, loading: destLoading } =
    useAvailableDestinations(origin || null, departureDate || null);
  ...
  const getStopDynamicCount = (stopId: string | number) => {
    ...
    if ((destData as any).times && departureDate) {
      ...
      const localISOToday = new Date(todayDate.getTime() - tzOffset)
        .toISOString()
        .slice(0, 10);
      const selectedISODate = new Date(departureDate)
        .toISOString()
        .slice(0, 10);
        ...
        const hasFinishedA =
          (
            availableDestinations.find(
              (d) => String((d as any).destinationId) === String(a.id),
            ) as any
          )?.times?.length > 0;
        const hasFinishedB =
          (
            availableDestinations.find(
              (d) => String((d as any).destinationId) === String(b.id),
            ) as any
          )?.times?.length > 0;
  ...
  useEffect(() => {
    ...
    if (!hasInitializedDefault.current) {
      const asuncion = stops.find(
        (s) =>
          s.name.toLowerCase().includes("asunción") ||
          s.name.toLowerCase().includes("asuncion"),
      );
      if (asuncion) {
        ...
        <div
          className={cn(
            ...
            orientation === "vertical"
              ...
              : "bg-white/20 border-black/15 dark:border-white/30",
          )}
        >
      ...
      >
        ...
        >
          ...
          <div className="flex lg:hidden justify-center mb-8 relative z-10">
            <div
              className={cn(
                "inline-flex backdrop-blur-sm rounded-full p-1 border",
                orientation === "vertical"
                  ? "bg-slate-200/50 dark:bg-slate-800/80 border-slate-300/50 dark:border-slate-700/50"
          ...
          <div
            className={cn(
              "flex flex-col gap-4 relative z-10 items-stretch w-full",
              orientation === "horizontal"
                ? "lg:flex-row lg:gap-3 lg:items-end"
                : "",
            )}
          >
            ...
            <div
              className={cn(
                "hidden flex-col flex-shrink-0",
                orientation === "horizontal" ? "lg:flex" : "",
              )}
            >
              <Label
                className={cn(
              ...
              <div
                className={cn(
                  "inline-flex h-14 items-center rounded-xl p-1 border shadow-sm",
                  orientation === "vertical"
            ...
            <div className="flex-1 min-w-0">
              <Label
                className={cn(
                  "text-lg sm:text-xl lg:text-[13px] font-extrabold lg:font-bold mb-2 lg:mb-1 block lg:[text-shadow:none]",
                  orientation === "horizontal" &&
                    "[text-shadow:_0_1px_3px_rgb(255_255_255_/_90%)]",
                  orientation === "vertical"
                    ? "text-slate-900 dark:text-white"
                    : "text-slate-900",
                )}
              ...
              >
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      orientation === "vertical"
                        ? "bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 dark:text-white"
                ...
                <PopoverContent
                  className={cn(
                    "w-[var(--radix-popover-trigger-width)] min-w-[280px] p-0 shadow-2xl rounded-2xl overflow-hidden",
                    orientation === "vertical"
                ...
                >
                  <Command className="bg-transparent">
                    <CommandInput
                      ...
                      className={cn(
                        "h-12 bg-transparent border-b transition-colors px-3 text-slate-900 dark:text-slate-200 border-slate-200 dark:border-slate-700 placeholder:text-slate-400",
                      )}
                    />
                    <CommandList className="max-h-72 overflow-y-auto p-1">
                      <CommandEmpty
                        className={cn(
                          "text-gray-500 p-4 text-sm text-center",
                          orientation === "vertical" && "dark:text-slate-400",
                        )}
                      >
                      ...
                      <CommandGroup className="bg-transparent">
                        {stops.map((city) => (
                          <CommandItem
                            ...
                            className={cn(
                              "cursor-pointer py-3 group transition-colors duration-150 rounded-lg px-3 flex items-center",
                              orientation === "vertical"
                                ? "text-slate-900 dark:text-slate-200 data-[selected=true]:bg-primary data-[selected=true]:text-slate-900 hover:bg-primary hover:text-slate-900"
                                : "text-slate-900 dark:text-slate-200 data-[selected=true]:bg-primary data-[selected=true]:text-slate-900 hover:bg-primary hover:text-slate-900",
                            )}
                          >
                            <MapPin
                              className="h-4 w-4 mr-2.5 text-primary shrink-0 group-hover:text-slate-900 group-data-[selected=true]:text-slate-900 transition-colors"
                              strokeWidth={2.2}
                            />
            ...
            >
              <button
                ...
                className={cn(
                  cn(
                    "w-10 h-10 flex items-center justify-center rounded-full shadow-sm transition-all duration-300 border",
                    orientation === "vertical"
            ...
            <div
              className={cn(
                "hidden items-center justify-center w-[52px] flex-shrink-0",
                orientation === "horizontal" ? "lg:flex" : "",
              )}
            >
                          ...
                          .map((city) => (
                            <CommandItem
                              ...
                              className={cn(
                                "cursor-pointer py-3 group transition-colors duration-150 rounded-lg px-3 flex items-center",
                                orientation === "vertical"
                                  ? "text-slate-900 dark:text-slate-200 data-[selected=true]:bg-secondary data-[selected=true]:text-black hover:bg-secondary hover:text-black"
                                  : "text-slate-900 dark:text-slate-200 data-[selected=true]:bg-secondary data-[selected=true]:text-black hover:bg-secondary hover:text-black",
                              )}
                            >
                              <MapPin
                                className="h-4 w-4 mr-2.5 text-secondary shrink-0 group-hover:text-black group-data-[selected=true]:text-black transition-colors"
                                strokeWidth={2.2}
                              />
                              <div className="min-w-0 flex-1">
                                ...
                                {(() => {
                                  ...
                                  const destData = availableDestinations.find(
                                    (d) => {
                                      if (typeof d === "string")
                                        return d === String(city.id);
                                      return (
                                        String((d as any).destinationId) ===
                                        String(city.id)
                                      );
                                    },
                                  );
                                  const isObject =
                                    destData && typeof destData === "object";
                                  const dynamicCount = getStopDynamicCount(
                                    city.id,
                                  );
                                  ...
                                  if (isObject) {
                                    if (dynamicCount > 0) {
                                      ...
                                      (destData as any).times?.length > 0
                                    ) {
                                      return (
              ...
              >
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <span
                        className={cn(
                          ...
                          cn(
                            "text-lg lg:text-base font-semibold truncate",
                            orientation === "vertical"
                              ? "text-slate-900 dark:text-white"
                              : "text-gray-900",
                          ),
                ...
                <PopoverContent
                  className={cn(
                    ...
                    orientation === "vertical"
                      ? "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700"
                      : "bg-white/95 border-black/20 text-slate-900 dark:text-slate-900",
                  )}
                  align="start"
                >
                  <CalendarComponent
            ...
            <div
              className={cn(
                "hidden flex-shrink-0 items-end ml-auto",
                orientation === "horizontal" ? "lg:flex" : "",
              )}
            >
              <Button
