...
export function SeatMap({ tripId, isReturn = false }: SeatMapProps) {
  ...
  if (loading) {
    return (
      <div className="bg-black/5 dark:bg-white/5 backdrop-blur-sm rounded-2xl p-12 flex flex-col items-center justify-center border border-black/10 dark:border-black/10 dark:border-white/20">
        <Loader2 className="h-10 w-10 text-primary animate-spin mb-4" />
        <p className="text-slate-900 dark:text-slate-900 dark:text-white/60">Cargando mapa de asientos...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-black/5 dark:bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-black/10 dark:border-black/10 dark:border-white/20">
  ...
  return (
    <div className="bg-black/5 dark:bg-white/5 backdrop-blur-sm rounded-2xl p-4 md:p-6 shadow-lg border border-black/10 dark:border-black/10 dark:border-white/20">
            ...
            <button
              ...
              className={cn(
                ...
                activeFloor === floor
                  ...
                  : "bg-black/10 dark:bg-white/10 text-slate-900 dark:text-slate-900 dark:text-white/80 hover:bg-black/20 dark:bg-white/20 border-black/15 dark:border-black/15 dark:border-white/30",
              )}
            >
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6 mb-8">
        <div className="flex items-center gap-1.5 md:gap-2">
          <div className="w-4 h-4 md:w-6 md:h-6 rounded bg-black/10 dark:bg-white/10 border border-black/15 dark:border-black/15 dark:border-white/30" />
        ...
        <div className="flex items-center gap-1.5 md:gap-2">
          ...
          <span className="text-xs md:text-sm text-slate-900 dark:text-slate-900 dark:text-white/60">Ocupado</span>
        </div>
        <div className="flex items-center gap-1.5 md:gap-2">
          <div className="w-4 h-4 md:w-6 md:h-6 rounded bg-black/5 dark:bg-white/5 border border-black/10 dark:border-black/10 dark:border-white/20 opacity-50" />
          <span className="text-xs md:text-sm text-slate-900 dark:text-slate-900 dark:text-white/60">
            No disponible
          </span>
        </div>
      ...
      <div className="relative max-w-md mx-auto">
        ...
        <div className="flex justify-center mb-4">
          <div className="w-32 h-12 bg-black/20 dark:bg-white/20 rounded-t-3xl flex items-center justify-center border border-black/15 dark:border-black/15 dark:border-white/30">
            <span className="text-slate-900 dark:text-slate-900 dark:text-white text-sm font-medium">
              Conductor
            </span>
          </div>
        </div>
        ...
        {/* Seats Grid */}
        <div className="bg-black/10 dark:bg-white/10 rounded-3xl p-6 border-4 border-black/15 dark:border-black/15 dark:border-white/30 relative">
          {/* Row Numbers */}
          <div className="absolute left-0 top-0 h-full flex flex-col pt-6 pb-6 pl-2">
            {rows.map((row, index) => (
              ...
              >
                <span className="text-xs font-medium text-slate-900 dark:text-slate-900 dark:text-white/60 w-4 text-center">
                  {row}
                </span>
              </div>
            ))}
        ...
        <div className="flex justify-center mt-4">
          <div className="w-48 h-8 bg-black/20 dark:bg-white/20 rounded-b-xl flex items-center justify-center border border-black/15 dark:border-black/15 dark:border-white/30">
            <span className="text-slate-900 dark:text-slate-900 dark:text-white/60 text-xs">Parte trasera</span>
          </div>
        </div>
      </div>

      {/* Selection Counter */}
      <div className="mt-8 p-4 bg-black/10 dark:bg-white/10 rounded-xl border border-black/10 dark:border-black/10 dark:border-white/20">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-slate-900 dark:text-slate-900 dark:text-white/60">Asientos seleccionados</p>
            <p
              className={cn(
            ...
            >
              ...
              <span className="text-base font-normal text-slate-900 dark:text-slate-900 dark:text-white/60 ml-1">
          ...
          {currentSelectedSeats.length > 0 && (
            <div className="text-right">
              <p className="text-sm text-slate-900 dark:text-slate-900 dark:text-white/60">Total seleccionado</p>
        ...
        {currentSelectedSeats.length > 0 && (
          <div className="mt-3 pt-3 border-t border-black/10 dark:border-black/10 dark:border-white/20">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isBlocked &&
          "bg-black/5 dark:bg-white/5 cursor-not-allowed border border-black/10 dark:border-black/10 dark:border-white/20 opacity-50",
        isDisabled &&
          !isOccupied &&
          !isBlocked &&
          !isSelected &&
        ...
        !isOccupied &&
          ...
          !isSelected &&
          !isDisabled &&
          "bg-black/10 dark:bg-white/10 border-2 border-black/15 dark:border-black/15 dark:border-white/30 hover:border-primary hover:bg-primary/10",
        isSelected &&
      ...
      {isOccupied ? (
        ...
        <>
          <span
            className={cn(
              "font-medium",
              isSelected ? "text-primary-foreground" : "text-slate-900 dark:text-slate-900 dark:text-white",
            )}
          >
            {seat.number}
          </span>
          ...
          <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50">
            <div className="bg-accent text-slate-900 dark:text-slate-900 dark:text-white-foreground text-xs px-2 py-1 rounded whitespace-nowrap shadow-lg border border-black/10 dark:border-black/10 dark:border-white/20">
              {isBlocked
                ? "No disponible"
                : isDisabled && !isOccupied && !isSelected
                  ? "Límite máximo"
