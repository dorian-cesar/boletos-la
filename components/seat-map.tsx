      ...
      <div className="relative max-w-md mx-auto">
        ...
        <div className="flex justify-center mb-4">
          <div className="w-32 h-12 bg-foreground rounded-t-3xl flex items-center justify-center">
            <span className="text-background text-sm font-medium">Conductor</span>
          </div>
        </div>
        ...
        <div className="bg-muted/50 rounded-3xl p-6 border-4 border-foreground/20">
          {rows.map(row => {
            const rowSeats = floorSeats.filter(s => s.row === row).sort((a, b) => a.column - b.column)
            const leftSeats = rowSeats.filter(s => s.column <= 2)
            const rightSeats = rowSeats.filter(s => s.column > 2)
            
            return (
              <div key={row} className="flex items-center justify-center gap-8 mb-3">
                {/* Left Side */}
                <div className="flex gap-2">
                  {leftSeats.map(seat => (
                    <SeatButton
                      key={seat.id}
                      seat={seat}
                      isSelected={currentSelectedSeats.some(s => s.id === seat.id)}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        !isOccupied && !isSelected && 'bg-muted border-2 border-border hover:border-primary hover:bg-primary/10',
        isSelected && 'bg-primary text-primary-foreground shadow-lg transform scale-110',
        seat.type === 'vip' && !isOccupied && !isSelected && 'border-secondary',
        seat.type === 'premium' && !isOccupied && !isSelected && 'border-primary/50'
      )}
    >
      {isOccupied ? (
