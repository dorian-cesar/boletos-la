...
export const useBookingStore = create<BookingState>()(
  persist(
    (set, get) => ({
      ...
      setSelectedOutboundTrip: (selectedOutboundTrip) => set({ selectedOutboundTrip }),
      setSelectedReturnTrip: (selectedReturnTrip) => set({ selectedReturnTrip }),
      ...
      removeReturnSeat: (seatId) => {
        ...
        set({ selectedReturnSeats: selectedReturnSeats.filter(s => s.id !== seatId) })
        get().calculateTotal()
      },
      ...
      calculateTotal: () => {
        const { selectedOutboundTrip, selectedReturnTrip, selectedSeats, selectedReturnSeats } = get()
        let total = 0
        ...
        if (selectedReturnTrip) {
          total += selectedReturnSeats.reduce((acc, seat) => acc + seat.price, 0)
        }
...
export const cities = [
  ...
  { id: 'la-serena', name: 'La Serena', region: 'Coquimbo' },
  { id: 'antofagasta', name: 'Antofagasta', region: 'Antofagasta' },
  { id: 'temuco', name: 'Temuco', region: 'Araucanía' },
  { id: 'puerto-montt', name: 'Puerto Montt', region: 'Los Lagos' },
  { id: 'iquique', name: 'Iquique', region: 'Tarapacá' },
  { id: 'talca', name: 'Talca', region: 'Maule' },
]

export const generateTrips = (origin: string, destination: string, date: string): Trip[] => {
  const times = ['06:00', '08:30', '10:00', '12:30', '15:00', '18:00', '21:00', '23:30']
  const companies = ['Turbus', 'Pullman Bus', 'Buses JAC', 'Eme Bus', 'Condor Bus']
  ...
  const amenities = [
    ...
    ['WiFi', 'TV', 'Baño', 'Aire Acondicionado', 'Enchufes USB', 'Comida incluida'],
    ['TV', 'Baño'],
  ]
