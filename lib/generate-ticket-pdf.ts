...
export async function generateTicketPDF(data: TicketData): Promise<Blob> {
  ...
  data.passengers.forEach((passenger, index) => {
    ...
    doc.setFontSize(11)
    doc.setFont('helvetica', 'bold')
    doc.text(`${index + 1}. ${passenger.firstName} ${passenger.lastName}`, 20, yPos + 7)
  ...
  doc.text(`$${data.totalPrice.toLocaleString('es-CL')}`, pageWidth - 20, yPos, { align: 'right' })
  ...
  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.text('Este boleto electrónico es válido como comprobante de viaje.', 20, footerY)
  doc.text('Presente su cédula de identidad al momento de abordar.', 20, footerY + 5)
  doc.text(`Generado el ${new Date().toLocaleDateString('es-CL')} a las ${new Date().toLocaleTimeString('es-CL')}`, 20, footerY + 10)
  
  doc.text('www.boletos.la | contacto@boletos.la | +56 2 2345 6789', pageWidth - 20, footerY + 10, { align: 'right' })
